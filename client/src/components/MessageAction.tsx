...
export function MessageAction({ orderId, order }: MessageActionProps) {
  const customerEmail = order.billing_address?.email || '';
  const [subject, setSubject] = useState(`Regarding Order #${order.id}`);
  ...
  async function handleSend() {
    ...
    setSuccess(false);
    try {
      await api.messageCustomer(orderId, { subject, message });
      setSuccess(true);
      setMessage('');
    } catch (err) {
  ...
  if (!customerEmail) {
    return (
      <Box>
        <InlineMessage
          type="warning"
          messages={[{ text: 'No customer email address found on this order. Cannot send a message.' }]}
        />
      </Box>
    );
  }
  ...
  return (
    <Box>
      <Text>Send a direct message to the customer about this order.</Text>
      <Box marginTop="small">
        <FormGroup>
          <Input
            label="To"
